...
public class GridCellTest {
	...
	private Seq wrapper(CoordinateSequence sequence, boolean closed) {
		return new Seq.Wrapper(sequence, 0, closed);
	}

	private <G extends Guide<?>> GridCell newMiddle(G lower, G upper, Seq seq) {
		Cell.Middle<G> cell = new Cell.Middle<>(lower, upper);
		return new GridCell(new Event.Factory(seq), cell);
	}

	private <G extends Guide<?>> GridCell newLower(G upper, Seq seq) {
		Cell.Lower<G> cell = new Cell.Lower<>(upper);
		return new GridCell(new Event.Factory(seq), cell);
	}

	private <G extends Guide<?>> GridCell newUpper(G lower, Seq seq) {
	...
	private PSequence<Event> traverse(Seq seq, GridCell cell,
	...
	public void x_middle_01() {
		Seq seq = wrapper(GeometryHelper.sequenceOf(sequence01), true);
